    ...
    <div className="space-y-4">
      ...
      <Card title="회의 설정">
        ...
        <div className="flex gap-2 items-end mb-2 flex-wrap">
          <Field label="날짜"><select value={meet.date||dates[0]||""} onChange={e=>setMeet({...meet,date:e.target.value})} className="p-2 border rounded-lg text-sm">{dates.map(d=><option key={d} value={d}>{d}</option>)}</select></Field>
              ...
              return(
                <div key={t.id} className="flex-1">
                  ...
                  <div className="flex gap-0.5">
                    ...
                    <button onClick={()=>toggle(key,"off",slot)} className={`flex-1 py-1 rounded text-[10px] ${cur==="off"?"bg-orange-400 text-white":"bg-slate-100 hover:bg-orange-100"}`}>휴가</button>
                  </div>
